...
- [第二阶段：深入敌后（碎镜爆发后第 31-90 天）](#第二阶段深入敌后碎镜爆发后第-31-90-天)
  ...
  - [第 9 章：Trait 对象与生命周期 · 武器的保质期](#第-9-章trait-对象与生命周期--武器的保质期)
  - [第 10 章：废墟里的日志](#第-10-章废墟里的日志)
- [第三阶段：锻造神兵（碎镜爆发后第 91-180 天）](#第三阶段锻造神兵碎镜爆发后第-91-180-天)
  - [第 11 章：互联网重启的第一声问候](#第-11-章互联网重启的第一声问候)
  - [第 12 章：并发反击](#第-12-章并发反击)
...
#### 1.14 `break` 和 `continue`<!-- omit in toc -->
...
#### 1.18 `Result` · 你拆开的每一个信封<!-- omit in toc -->
...
#### 1.20 接收用户输入 · 让程序和人说话<!-- omit in toc -->
...
#### 7.7 完整的错误处理版本<!-- omit in toc -->

以下是 `nightwatch_core/src/dict.rs` 最终版本：
...
意思很直接：“你让我用 `{:?}` 打印 `T`，但我不确定 `T` 是不是能打印的类型。”

因为 `T` 目前是“任意类型”。万一将来有人传进来一个不能打印的东西怎么办？

所以你要给 `T` 加一个约束：

```rust
fn print_pair<T: std::fmt::Debug>(a: T, b: T) {
    println!("{:?} 和 {:?}", a, b);
}
```
...
这句话现在你可能只是照着写。等你看完 8.2 节，你会完全明白它。

##### 8.1.3 泛型用在结构体上<!-- omit in toc -->
...
##### 8.1.4 泛型解决的是什么问题？<!-- omit in toc -->

**消除重复代码。**

没有泛型的时候，你为每种类型写一份几乎一样的函数。有泛型之后，你写一份，编译器帮你生成所有类型需要的版本。

**注意：** 泛型不是“偷懒”（众所周知，“懒”是极客的一大美德）。它是“把重复的逻辑抽出来，只写一次”。

---

#### 8.2 trait——给类型贴标签<!-- omit in toc -->

你在第 5 章已经见过 trait 了，只是当时没有专门说明。
...
然后就能用 `println!("{:?}", report)` 打印它。

为什么？因为 `Debug` 是一个 trait。你给 `ThreatReport` 贴上了 `Debug` 这个标签，等于告诉 Rust：“这个类型可以被调试打印。”
...
你之前用 `#[derive(...)]`，就是让 Rust 自动帮你贴标签。

---

#### 8.3 自己定义 trait<!-- omit in toc -->

标准库提供的标签是有限的。有时候你想自己定义一种“能力”。
...
- `trait`：我要定义一个新标签
- `Attack`：标签名字叫“攻击”
- `{ fn attack(&self); }`：任何贴这个标签的类型，必须实现一个叫 `attack` 的方法

注意：这里**只有方法签名，没有函数体**。它只说“必须有什么方法”，不说“具体怎么实现”。

这就像一张报名表，上面写着：“想加入守夜者，~~必须能跑 25 公里~~必须有最基本的道德。”
...
同一个标签 `Attack`，同一个方法名 `attack`，但具体行为由每个类型自己决定。

---

#### 8.4 用 trait 约束函数参数<!-- omit in toc -->
...
`println!("{:?}", value)` 要求 `value` 的类型实现了 `Debug` trait。

你写泛型函数 `fn print_pair<T>(a: T, b: T)` 时，`T` 可能是任何类型。编译器不能保证它实现了 `Debug`。所以它拒绝编译。
...
> “这个函数接受类型 `T`。`T` 可以是什么都行，但必须实现了 `Debug`。因为我后面要用 `{:?}` 打印它。”

---

#### 8.6 燃烧者笔记<!-- omit in toc -->

> “泛型是你的通用钥匙。trait 是你的标签系统。你写泛型函数时，编译器不让你打印一个没有 `Debug` 标签的类型——这不是它在找茬，是它在帮你提前排掉一颗雷。”
>
...
你有不同种类的武器：`String` 能攻击，`i32` 能攻击，将来还会有结构体能攻击。它们都贴了 `Attack` 标签。

但当你把它们放进同一个武器架时，编译器不干了。
...
```rust
let weapons = vec![String::from("守夜者"), 42];
```

编译失败。因为 `String` 和 `i32` 是两种不同的具体类型。
...
不行。因为第一个是 `String`，第二个是 `i32`。

这是起点。你在这里卡住了。

**第二步：换个角度看“它们有什么共同点”**  

`String` 和 `i32` 是两种完全不同的类型。
...
意味着你可以对 `String` 调用 `.attack()`，也可以对 `i32` 调用 `.attack()`。
...
所以它们虽然在“具体类型”上不同，但在“能不能 attack”这件事上是相同的。

你现在需要的，是一种方式，**只看“能不能 attack”这一层，忽略它们具体是什么类型。**
...
> **“某种实现了 Attack trait 的类型。具体是 String 还是 i32，我现在不关心。”**
...
你不能把“张三”和“李四”说成是同一个人。但你可以把他们都归进“会开枪的人”这个类别里。
...
想想看：`String` 在内存里占一块地方，`i32` 只占 4 个字节。它们大小不同。如果你把它们都当成 `dyn Attack`，编译器根本不知道要给每个元素预留多少空间。
...
你可能会问：`&String` 和 `&i32`，真的能放进 `Vec<&dyn Attack>` 里吗？

能。

因为 Rust 会做一次自动转换：把 `&String` 转成 `&dyn Attack`，把 `&i32` 也转成 `&dyn Attack`。
...
目前你只需要用 `&dyn Attack`。`Box` 会在第 13 章详细讲。
...
这个例子很简单。`s` 在函数内部创建，函数结束就销毁。你返回它的地址，就是指向空地。
...
这个函数接收两个引用，返回其中较长的那个。

`x` 和 `y` 都是**外部传进来的**，不是函数内部创建的。但编译器不知道：**返回值到底指向 `x`，还是 `y`？**

如果 `x` 活得短，`y` 活得长，而返回值指向 `x`——那调用者在 `x` 销毁之后还在用返回值，就崩了。

编译器需要你说明：“返回的引用，和传入的引用之间是什么关系。”
...
1. **函数接收多个引用，返回其中一个引用** → 你要告诉编译器返回值和哪个输入参数有关
2. **结构体里存了引用** → 你要告诉编译器这个引用和结构体之间的存活关系
...
意思是：“`Message` 里的 `content` 引用，至少要和 `Message` 本身活得一样久。”

##### 9.2.5 你只需要记住三条<!-- omit in toc -->
...
>
> “我第一次用 `dyn` 的时候，觉得它像给所有武器套了同一个外壳。后来发现，它只是把‘实际类型’藏在了一块统一的门牌号后面——运行时再进去找。”
>
> “生命周期是最容易让新手放弃 Rust 的东西。但说真的，你不需要一次全懂。你只需要看懂编译器的意思：它在问你，‘你返回的这个引用，到底是谁给你的？’”
...
**技术提示：** 如果 `&"燃烧者"` 不能直接放进 `Vec<&dyn Attack>`，试试先创建一个 `let s: &str = "燃烧者";`，然后把 `&s` 放进去。或者把 `&"燃烧者"` 改成 `"燃烧者"`。仔细看编译器给你的信息。

---

##### 练习二：编写你的第一个带生命周期标注的函数<!-- omit in toc -->
