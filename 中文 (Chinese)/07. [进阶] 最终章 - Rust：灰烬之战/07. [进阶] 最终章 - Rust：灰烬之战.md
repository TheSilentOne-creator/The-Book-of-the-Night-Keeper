...
> *想要完整的 `attacks.log` 可以去 GitHub 仓库下的 `code` 文件夹里获取。*
...
> “`127.0.0.1` 是你自己的电脑。`1234` 是端口号”

但代码里是 `1234`，不是 `1234`。

然后 11.5 燃烧者笔记里又写：

> “用浏览器访问 `http://localhost:1234`”

这里又变回 `1234`。

读者如果照着做，会很恍惚：我到底该访问 1234 还是 1234？为什么代码里是 1234，笔记里是 1234？这不是小节问题，这会直接让读者卡在“浏览器打不开页面”上。你之前每一章的教学都建立在“让读者亲手跑通”的基础上。端口号不一致，直接破坏了这个基础。

所以第一件事：**全章端口号统一成 `1234`，或者统一成 `1234`。** 我建议用 `1234`，因为这一章你自己写的就是 1234，而且 1234 比 1234 好记，和《The Book》原本的 1234 区分开，更有“守夜者自己的服务器”的感觉。

---

第二个问题：这一章“恍惚”的另一个来源，是**你在 11.1 里把一个地址写错了，但没发现**。

你说：

> `TcpListener::bind("127.0.0.1:1234")?` —— 告诉系统："我要在 1234 端口上站岗。"

但你代码里是 1234。这个口误和上面的端口号问题叠加在一起，会让读者觉得“我是不是看漏了什么”。

---

第三个问题：**11.2 读取请求部分，你多写了一个“请求体”的说明，但服务器根本没读请求体。**

你在 11.2 开头说：

> HTTP 协议里，浏览器向服务器发送一个"请求"——它是一段纯文本，以三部分组成：请求行、请求头、请求体。

然后你只读了请求行。

读者会困惑：你说了三部分，为什么只读一行？请求头和请求体呢？

这个问题不是硬伤，但它会让这一章“飘”。你要么把三部分的说明砍掉，只留“第一行是请求行，我们先读这一行”；要么在最后加一句“请求头和请求体，等以后需要的时候再读。现在只需要知道第一行是什么”。

---

第四个问题：**`Content-Length: 18` 是写死的，但“守夜者，我在。”的长度不是 18 字节。**

“守夜者，我在。”如果按 UTF-8 编码，中文字符占 3 个字节，逗号和句号各占 3 个字节，总共是：
...
如果你不打算让读者去深究字节数，就不要用中文正文配固定 `Content-Length`。要么把响应体换成英文 `Keeper is here.`（18 字节刚好），要么直接去掉 `Content-Length` 这一行，让浏览器自己判断。对于这一章的教学目标来说，去掉 `Content-Length` 是最干净的。

---

第五个问题：**11.3 的代码没有读请求，直接写响应。**

你在 11.2 刚教了怎么读请求行，结果 11.3 的代码里，把读请求那几行又删掉了。读者读到这里会恍惚：11.2 学的东西，11.3 为什么不用了？

这不是错，但教学上很尴尬。11.3 应该是 11.2 的“增量”，而不是“另起炉灶”。如果 11.3 的完整代码里直接省略了读请求，读者会觉得前面的东西白学了。

---

所以，你感觉“恍惚”是完全对的。这一章有五个点让读者会卡：

1. 端口号 `1234` 和 `1234` 打架
2. 正文里说 1234，代码里是 1234
3. 说了请求三部分，只读了一部分
4. `Content-Length: 18` 和中文正文字节数不符
5. 11.3 丢了 11.2 的读请求代码

---

这些都是小节，但小节会毁掉一整章的信任感。教程写作里最可怕的不是“讲错了”，而是“让读者不知道是不是自己看错了”。

我把这一章按“最小一致”原则重写一遍。端口号统一用 `1234`，响应体换成英文 `Keeper is here.`（正好 18 字节，保留 `Content-Length`），11.3 的代码保留读取请求行，并且让请求三部分的说明只落在“第一行是请求行”上。

---

好的，我来改。这一章的关键是：**先让任务把读者拽进来，再抒情；先让读者理解响应是什么，再拼出那行代码。** 最后加一个简短的“你现在的状态”和课后作业。
...
> “我找到一台旧笔记本，能开机。你装好 Rust。我要你做一件事：写一个程序，让这台笔记本变成一座灯塔。”
>
...
把 `stream` 的所有权从主线程移进新线程。这样主线程在继续等下一个连接时，`stream` 已经被新线程拿走了，不会出现两个线程同时使用同一个 `stream` 的问题。

**主线程不等待**  

`thread::spawn` 之后，主线程立刻回到 `listener.incoming()`，继续等下一个请求。处理请求的工作由新线程完成。

**测试它。** 在另一个终端里同时发 10 个请求：

```bash
for i in {1..10}; do curl http://127.0.0.1:1234 & done
```

终端会打印 10 行“收到请求”。程序没有崩。同时处理了 10 个请求。
...
Rust 的通道叫 `mpsc`，是 **Multiple Producer Single Consumer** 的缩写。
...
创建一个通道。返回一个元组：`(tx, rx)`。`tx` 是发送端，`rx` 是接收端。
...
从通道里取数据。如果通道里没有数据，`recv()` 会阻塞当前线程，一直等到有数据到达。
...
`Arc` 和 `Mutex` 是两个不同的工具。它们总是配对使用。

**`Arc<T>`：让多个线程共享同一个数据。**

`Arc` 是 **Atomically Reference Counted** 的缩写。它会记录“有多少个地方在引用这个数据”。当最后一个引用被丢弃时，数据才被销毁。
...
// data、data1、data2 都指向同一个 42
```

`Arc` 是线程安全版的 `Rc`。你在第 8 章没见过 `Rc`，但以后会见到。现在只需要记住：**要在多线程里共享一个数据，用 `Arc`。**

**`Mutex<T>`：保证同一时间只有一个线程能访问数据。**

`Mutex` 是 **Mutual Exclusion** 的缩写。它是一把锁。一个线程进去，锁门；出来，开门。其他线程只能等。
...
`Arc` 让多个线程共享一个 `Mutex`。`Mutex` 让同一时间只有一个线程能碰里面的数据。
...
`rx` 被包进 `Arc<Mutex<>>`。每个工作线程拿一个 `Arc::clone(&rx)`，就能安全地从通道里取任务。
