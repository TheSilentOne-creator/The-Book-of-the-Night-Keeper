...
你已经写了 100 天的 Rust 代码。

你读了字典，发了请求，解析了日志，拆了模块，理解了所有权。你的工具箱从一把螺丝刀变成了一座小型军火库。

但有一件事你一直没做过。

你写的所有程序，都是你敲命令、它做事，然后结束。输出永远是终端里的文字——白色字符在黑色背景上滚过去，滚完了就没了。

你从来没有让一个程序**一直开着**，像一座灯塔一样站在那里，等人来问它"你在不在"。

碎镜爆发后的第 100 天，你决定做这件事。

你用旧零件拼凑了一个最原始的 Web 服务器。它的硬件是一台和你同龄的旧笔记本。它的代码是一段几十行的 Rust 程序。它的任务只有一个：监听一个端口，当有人访问时，大声说"守夜者，我在。"

然后你打开浏览器，输入 `http://localhost:1234`，按下回车。

你第一次在浏览器里看到了自己的程序跑出来的内容。

只有一行字。

但那行字，是碎镜爆发后，你第一次觉得"互联网"还活着。

因为互联网的本质，就是一台机器在等待，另一台机器问了一句话，然后第一台机器回答了。

这就是互联网重启的第一声问候。

---

#### 11.1 用 `TcpListener` 开门——让你的程序等人敲门<!-- omit in toc -->

你之前的程序都是"启动→做事→退出"的流程。`cargo run` 之后，它跑几秒就结束了。
...
`TcpListener::bind("127.0.0.1:1234")?` —— 告诉系统："我要在 7878 端口上站岗。有人连这个端口，通知我。"

`127.0.0.1` 是你自己的电脑。`7878` 是端口号，一个未被占用的数字。

`listener.incoming()` —— 返回一个迭代器，每次有新的连接进来，它就吐出一个 `TcpStream`（代表"这个连接"）。

`for stream in listener.incoming()` —— 循环等待。没人来的时候，程序停在这里不动。有人来了，执行一次循环体。

`stream?` —— 连接可能失败（比如对方中途断开了），`?` 帮你跳过失败的连接，继续等下一个。
...
HTTP 协议里，浏览器向服务器发送一个"请求"——它是一段纯文本，以三部分组成：

1. **请求行**：`GET / HTTP/1.1`（用什么方法、要什么资源、用什么版本）
2. **请求头**：一大堆 `Key: Value`（告诉服务器浏览器的信息）
3. **请求体**：如果有数据（比如 POST 表单），放在这里

浏览器访问 `http://127.0.0.1:1234` 时发送的第一行是：
...
`reader.lines().next()` —— 读取请求的第一行。浏览器发来的请求中，第一行就是"你要什么"。
...
- 第一行：状态行（版本 + 状态码 + 状态描述）
- 第二行：`Content-Length`，告诉浏览器"响应体有 18 个字节"
- 一个空行（告诉浏览器"头结束了，下面是正文"）
- 正文（实际显示的内容）
...
`stream` 是 `TcpStream`——你可以往里面写数据，数据通过网络传给浏览器。

`stream.write_all(response.as_bytes())?` —— 把响应发回去。

`stream.flush()?` —— 确保数据已经全部发送，不是留在缓冲区里等。
...
>
> "第一次看到'守夜者，我在。'出现在浏览器里，你会有一种奇怪的感觉——好像在说'你做到了'。不是代码跑通了，是这个世界还活着。"
