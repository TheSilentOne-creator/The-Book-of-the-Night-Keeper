...
> “`127.0.0.1` 是你自己的电脑。`7878` 是端口号”

但代码里是 `1234`，不是 `7878`。

然后 11.5 燃烧者笔记里又写：

> “用浏览器访问 `http://localhost:7878`”

这里又变回 `7878`。

读者如果照着做，会很恍惚：我到底该访问 1234 还是 7878？为什么代码里是 1234，笔记里是 7878？这不是小节问题，这会直接让读者卡在“浏览器打不开页面”上。你之前每一章的教学都建立在“让读者亲手跑通”的基础上。端口号不一致，直接破坏了这个基础。

所以第一件事：**全章端口号统一成 `1234`，或者统一成 `7878`。** 我建议用 `1234`，因为这一章你自己写的就是 1234，而且 1234 比 7878 好记，和《The Book》原本的 7878 区分开，更有“守夜者自己的服务器”的感觉。
...
> `TcpListener::bind("127.0.0.1:1234")?` —— 告诉系统："我要在 7878 端口上站岗。"
...
1. 端口号 `1234` 和 `7878` 打架
2. 正文里说 7878，代码里是 1234
3. 说了请求三部分，只读了一部分
