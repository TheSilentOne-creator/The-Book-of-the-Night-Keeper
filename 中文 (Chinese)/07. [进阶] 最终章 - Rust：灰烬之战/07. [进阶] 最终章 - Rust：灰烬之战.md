...
你真正用过 `unwrap()` 的地方是第 2 章的 `load_dict` 函数——打开文件那行。那时候你觉得"反正文件就在那里"。
...
```rust
fn try_login(client: &Client, pin: &str, target: &str) -> Result<bool, reqwest::Error> {
    let response = client.post(target).send()?;  // 这里
    Ok(response.status().is_success())
}
```

你早就用过了，只是没有专门解释过它是什么。
...
#### 7.9 你现在的状态<!-- omit in toc -->

- [x] 你知道 `Result` 是"可能失败的操作"的返回值
- [x] 你知道 `unwrap()` 在失败时会崩溃，而且不告诉你原因
- [x] 你知道 `expect()` 在失败时会崩溃，但留下一条自定义信息
- [x] 你知道 `?` 把错误传给调用者，让上级决定怎么处理
- [x] 你知道什么时候用 `panic!` 主动崩溃
- [x] 你会把函数设计成返回 `Result`，把"怎么处理错误"交给调用者
